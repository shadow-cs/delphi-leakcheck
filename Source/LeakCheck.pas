...
var
  LTypeInfo: PTypeInfo absolute ATypeInfo;
  ...
  MethodAddr: Pointer;
begin
  // Gather all memory bock dependent data here
  // Prevent memory block info overwrite
  CS.Enter;
  try
    // Get virtual index stored as ordinal range of our special types
    Assert(LTypeInfo^.Kind = tkInteger);
    PByte(LTypeData) := PByte(@LTypeInfo.NameLength) + LTypeInfo.NameLength + 1;
    Assert(LTypeData^.MinValue = LTypeData^.MaxValue);
    VirtualIndex := LTypeData^.MinValue;
    Dec(VirtualIndex, ((- vmtParent) div SizeOf(Pointer)) - 1);
    Rec := TLeakCheck.ToRecord(Self);
    // Get original class
    LClassType := Rec^.PrevClass;
    if IsValidClass(LClassType) then
    begin
...
{$IFEND}
    ...
    try
      Buff.EnsureFree(512);
      StrCat(Buff, 'An attempt to call a virtual method on a freed object. An access violation will now be raised in order to abort the current operation.');
      StrCat(Buff, sLineBreak);
      StrCat(Buff, 'Class type: ');
      if Assigned(LClassData) then
        StrCat(Buff, @LClassData^.ClassName.Data, LClassData^.ClassName.Length)
      else
        StrCat(Buff, 'unknown (memory info block corrupted)');
      StrCat(Buff, sLineBreak);
      StrCat(Buff, 'Virtual method index: ');
      StrCat(Buff, IntToStr(VirtualIndex));
      StrCat(Buff, sLineBreak);
      StrCat(Buff, 'Virtual method address: ');
      if Assigned(MethodAddr) then
        StrCat(Buff, IntToStr(NativeUInt(MethodAddr), SizeOf(Pointer) * 2, 16))
      else
        StrCat(Buff, 'unknown');
      Name := nil;
  {$IF TLeakCheck.MaxStackSize > 0}
      ...
      begin
        InitializeStackFormatter;
        if FStackTraceFormatter.FormatLine(MethodAddr, @NameBuffer[0],
          Length(NameBuffer)) > 0 then
        begin
          Name := @NameBuffer[0];
        end;
      end;
  ...
  {$WARN SYMBOL_DEPRECATED OFF}
      if not Assigned(Name) then case VirtualIndex  * SizeOf(Pointer) of
  {$IFDEF AUTOREFCOUNT}
        vmtObjAddRef :
          Name := '_ObjAddRef';
        vmtObjRelease :
  ...
  try
{$IF Defined(MSWINDOWS) AND NOT Defined(NO_MESSAGEBOX)}
    MessageBoxA(0, Buff, 'Virtual call error', MB_ICONERROR or MB_OK);
{$ELSE}
    ReportLeak(Buff);
{$IFEND}
  finally
